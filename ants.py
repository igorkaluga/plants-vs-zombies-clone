    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container and place.ant.can_contain(self):
                # container already placed, place ant into place.ant
                place.ant.store_ant(self)
            elif self.is_container and self.can_contain(place.ant):
                # place container on place.ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place_count = 0 # own place
        while not cur_place.is_hive:
            if cur_place.bees and self.min_range <= place_count <= self.max_range:
...
class SlowThrower(ThrowerAnt):
    """Slows a bee for 3 turns, can only move on even turns when slowed. Can stack"""
...
class ScaryThrower(ThrowerAnt):
    """Scares a bee causing it to go backwards. Can only scare once"""
    ...
    def throw_at(self, target):
        if not target.is_scared and target:
            target.scare(2)
...
# The WallAnt class
class WallAnt(Ant):
    """Wall ant does nothing but act as a wall"""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """Eats a random whole bee from its place, chews for 3 turns"""
    ...
    def action(self, gamestate):
        if self.chew_countdown == 0:
            # eat a bee if there are bees
            if self.place.bees:
                chosen_one = random_bee(self.place.bees)
                chosen_one.reduce_health(chosen_one.health)
...
class TankAnt(ContainerAnt):
    """Tank ant that stores an ant but also deals 1 damage to all ants in its place"""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower ant can be in water, acts as a regular thrower ant."""
    food_cost = 6
    is_waterproof = True
    implemented = True
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 12
        #throws leaf, perforams action of ScubaThrower
        super().action(gamestate)
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        elif self.slowed_amt > 0 and gamestate.time % 2 == 1:
            # dont move because slowed and gamestate is odd
            self.slowed_amt -= 1
            pass
        elif self.slowed_amt > 0 and gamestate.time % 2 == 0:
            # can move because gamestate is even
            self.slowed_amt -= 1
            # move backwards if scared
            if self.scared_amt != 0:
                self.scared_amt -= 1
                self.move_to(self.place.entrance)
            else:
                if self.health > 0 and destination is not None:
                    self.move_to(destination)
        elif self.scared_amt != 0:
            self.scared_amt -= 1
            self.move_to(self.place.entrance)
        else:
            if self.health > 0 and destination is not None:
